...
from tensorflow.keras.layers import Layer, Input, Conv2D, Activation, add, BatchNormalization, UpSampling2D, ZeroPadding2D, Conv2DTranspose, Flatten, MaxPooling2D, AveragePooling2D, InputSpec
...
class CycleGAN():
    def __init__(self, lr_D=2e-4, lr_G=2e-4, image_shape=(256*1, 256*1, 1),
                 date_time_string_addition='_test', image_folder='mrict'):
        ...
        self.lambda_D = 10  # Weight for loss from discriminator guess on synthetic images
    ...
    def load_model_and_generate_synthetic_images(self):
        ...
        if response == 'y':
            ...
            def save_image(image, name, domain):
                ...
                image_max = np.max(image)
                image_scaled = 255 * (image - image_min) / (image_max - image_min)
                path_name = os.path.join('generate_images', 'synthetic_images', domain, name)
                cv2.imwrite(path_name, image_scaled)
