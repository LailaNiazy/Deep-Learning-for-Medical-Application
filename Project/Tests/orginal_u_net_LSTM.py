...
def orginal_u_net_lstm():
    
    #########Initialize the data for the images#############
    img_h = 240 #image size
    img_w = 240 #image size
    img_ch = 1
    number_of_labels = 1
    ...
    model = u_net_lstm(base,img_h, img_w, img_ch, batch_normalization, SDRate, spatial_dropout, number_of_labels)
    ...
    for train_index, test_index in cv.split(images):
            ...
            History = model.fit_generator(train_datagen.flow(x_train, y_train,batch_size = batch_size), validation_data = val_datagen.flow(x_val,y_val), epochs = epochs, verbose = 1, class_weight=weights)      
