...
def task_2():
    ...
    for train_index, test_index in cv.split(images):
        ...
        #Build the model
        model, input_weight = u_net(base,image_size, image_size, img_ch, batch_normalization, SDRate, spatial_dropout,final_neurons, final_afun,True)
        model.summary()
        
        #Compile the model
        model.compile(optimizer = Adam(lr=LR), loss = weighted_dice_loss(input_weight, weight_strength), metrics =[dice_coef, Recall(), Precision()] )
        ...
        if data_augmentation:
            ...
            History = model.fit_generator(train_generator, epochs=epochs, 
                                                ...
                                                validation_data=([x_val, weight_val], y_val), shuffle=True, class_weight='auto',steps_per_epoch = math.ceil(len(x_train) / batch_size))
        else:
